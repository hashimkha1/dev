...
def companyagenda(request):
    ...
    return render(
        request,
        "management/companyagenda.html",
        {"title": "Company Agenda", "data": data},
    )
    ...
    if request.method == "POST":
        ...
        for emp in employee:
            for act in activitys:
                if (
                    Task.objects.filter(category_id=category, activity_name=act).count()
                    > 0
                ):
        ...
        employess = User.objects.filter(
            Q(is_employee=True) | Q(is_admin=True) | Q(is_superuser=True)
        ).all()

    return render(
        request,
        "management/tasknew_form.html",
        {"category": tag, "employess": employess},
    )
...
def newevidence(request, taskid):
    if request.method == "POST":
        ...
        if form.is_valid():
            points, maxpoints, taskname = Task.objects.values_list(
                "point", "mxpoint", "activity_name"
            ).filter(id=taskid)[0]
...
def userevidence(request, user=None, *args, **kwargs):
    ...
    employee = get_object_or_404(User, username=kwargs.get("username"))
    userlinks = (
        TaskLinks.objects.all().filter(added_by=employee).order_by("-created_at")
    )
...
def AdvertisementView(request):
    ...
    if request.method == "POST":
        if t_form.is_valid() and f_form.is_valid():
            ...
            print("Error: Invalid Forms")
            return redirect("management:advertisement")

    return render(
        request, "management/advertisement.html", {"t_form": t_form, "f_form": f_form}
    )
